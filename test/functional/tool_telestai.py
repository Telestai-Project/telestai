...
"""Test the telestai wrapper tool."""
...
class ToolBitcoinTest(BitcoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `telestai node -version` is
        # run on windows, python doesn't capture output from the child
        # `telestaid` and `telestai-node` process started with _wexecvp, and
        # stdout/stderr are always empty. See
        # https://github.com/Telestai-Project/telestai/pull/33229#issuecomment-3265524908
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through telestai wrapper command with specified arguments."""
        node.args = [self.binary_paths.telestai_bin] + args + ["node"] + self.node_options[node.index]
    ...
    def run_test(self):
        ...
        self.log.info("Ensure telestai node command invokes telestaid by default")
        self.test_args([], [], expect_exe="telestaid")

        self.log.info("Ensure telestai -M invokes telestaid")
        self.test_args(["-M"], [], expect_exe="telestaid")

        self.log.info("Ensure telestai -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure telestai -m invokes telestai-node")
            self.test_args(["-m"], [], expect_exe="telestai-node")

            self.log.info("Ensure telestai -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="telestai-node")

            self.log.info("Ensure telestai accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="telestai-node")

            self.log.info("Ensure telestai recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="telestai-node")
