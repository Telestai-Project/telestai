...
"""Test the meowcoin wrapper tool."""
...
class ToolBitcoinTest(BitcoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `meowcoin node -version` is
        # run on windows, python doesn't capture output from the child
        # `meowcoind` and `meowcoin-node` process started with _wexecvp, and
        # stdout/stderr are always empty. See
        # https://github.com/meowcoin/meowcoin/pull/33229#issuecomment-3265524908
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through meowcoin wrapper command with specified arguments."""
        node.args = [self.binary_paths.meowcoin_bin] + args + ["node"] + self.node_options[node.index]
    ...
    def run_test(self):
        ...
        self.log.info("Ensure meowcoin node command invokes meowcoind by default")
        self.test_args([], [], expect_exe="meowcoind")

        self.log.info("Ensure meowcoin -M invokes meowcoind")
        self.test_args(["-M"], [], expect_exe="meowcoind")

        self.log.info("Ensure meowcoin -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure meowcoin -m invokes meowcoin-node")
            self.test_args(["-m"], [], expect_exe="meowcoin-node")

            self.log.info("Ensure meowcoin -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="meowcoin-node")

            self.log.info("Ensure meowcoin accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="meowcoin-node")

            self.log.info("Ensure meowcoin recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="meowcoin-node")
