...
class SegWitTest(TelestaiTestFramework):
    ...
    def test_unnecessary_witness_before_segwit_activation(self):
        ...
        # Right now, telestaid delays sending reject messages for blocks
    ...
    # To enable this test, pass --oldbinary=<path-to-pre-segwit-telestaid> to
    # the test.
    def test_upgrade_after_activation(self, node_id):
        self.log.info("Testing software upgrade after softfork activation")

        assert (node_id != 0)  # node0 is assumed to be a segwit-active telestaid
    ...
    def test_non_standard_witness(self):
        ...
        for i in range(len(scripts)):
            ...
            p2wsh_tx.vout.append(CTxOut(outputvalue - 351, CScript([OP_0, hash160(hex_str_to_bytes(""))])))
            p2wsh_tx.wit.vtxinwit.append(CTxInWitness())
            p2wsh_tx.rehash()
            p2wsh_txs.append(p2wsh_tx)
            p2sh_tx = CTransaction()
            p2sh_tx.vin.append(CTxIn(COutPoint(txid, i * 2 + 1), CScript([p2wsh_scripts[i]])))
            p2sh_tx.vout.append(CTxOut(outputvalue - 351, CScript([OP_0, hash160(hex_str_to_bytes(""))])))
