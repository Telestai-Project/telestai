...
def get_first_unspent(self: object, node: object, needed: float = 50.1) -> object:
...
def get_tx_issue_hex(self, node, asset_name, asset_quantity, asset_units=0):
    ...
    outputs = {
        'n1issueAssetXXXXXXXXXXXXXXXXWdnemQ': 50,
        change_address: truncate(float(unspent['amount']) - 50.1),
        to_address: {
            'issue': {
...
class RawAssetTransactionsTest(TelestaiTestFramework):
    ...
    def reissue_tampering_test(self):
        ...
        outputs = {
            'n1ReissueAssetXXXXXXXXXXXXXXWG9NLd': 10,
            n0.getnewaddress(): truncate(float(unspent['amount']) - 10.1),
            to_address: {
                'reissue': {
                    'asset_name':       asset_name,
                    'asset_quantity':   1000,
    ...
    def issue_reissue_transfer_test(self):
        ...
        outputs = {
            'n1ReissueAssetXXXXXXXXXXXXXXWG9NLd': 10,
            change_address: truncate(float(unspent['amount']) - 10.1),
            to_address: {
                'reissue': {
                    'asset_name':       'TEST_ASSET',
                    'asset_quantity':   1000,
    ...
    def bad_ipfs_hash_test(self):
        ...
        outputs = {
            'n1issueAssetXXXXXXXXXXXXXXXXWdnemQ': 50,
            change_address: truncate(float(unspent['amount']) - 50.0001),
            to_address: {
                'issue': {
                    'asset_name':       asset_name,
                    'asset_quantity':   1,
        ...
        outputs = {
            'n1ReissueAssetXXXXXXXXXXXXXXWG9NLd': 10,
            change_address: truncate(float(unspent['amount']) - 10.0001),
            to_address: {
                'reissue': {
                    'asset_name':       asset_name,
                    'asset_quantity':   1000,
    ...
    def atomic_swaps_test(self):
        ...
        n0.sendtoaddress(receive1, 35100)
        n0.generate(1)
        self.sync_all()
        ...
        n0.sendtoaddress(receive2, 35100)
        n0.generate(1)
        self.sync_all()
    ...
    def fundrawtransaction_nonwallet_transfer_outs(self):
        ...
        # fund n2
        n0.sendtoaddress(n2_address, 100)
        n0.generate(1)
        self.sync_all()
