...
class KeyPoolTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        res = w2.walletcreatefundedpsmt(inputs=[], outputs=[{destination: 0.00008900}], feeRate=0.00010)
        assert_equal("psmt" in res, True)

        # dust change should be removed
        res = w2.walletcreatefundedpsmt(inputs=[], outputs=[{destination: 0.00008800}], feeRate=0.00010)
        assert_equal("psmt" in res, True)

        # create a transaction without change at the maximum fee rate, such that the output is still spendable:
        res = w2.walletcreatefundedpsmt(inputs=[], outputs=[{destination: 0.00010000}], subtractFeeFromOutputs=[0], feeRate=0.0008823)
        assert_equal("psmt" in res, True)
