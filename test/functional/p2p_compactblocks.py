...
# TestNode: A peer we use to send messages to telestaid, and store responses.
...
class CompactBlocksTest(TelestaiTestFramework):
    ...
    # This test actually causes telestaid to (reasonably!) disconnect us, so do this last.
    ...
    # Test that telestaid requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, node, test_node, version):
        ...
        # Note that it's possible for telestaid to be smart enough to know we're
    ...
    def test_getblocktxn_handler(node, test_node, version):
        # telestaid will not send blocktxn responses for blocks whose height is
