...
def main():
    ...
    enable_meowcoind = config["components"].getboolean("ENABLE_BITCOIND")

    if not enable_meowcoind:
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if meowcoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "meowcoind"], stdout=subprocess.DEVNULL).returncode == 0:
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Meowcoin functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/meowcoin/meowcoin/pull/27561.
