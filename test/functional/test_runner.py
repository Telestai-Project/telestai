...
BASE_SCRIPTS = [
    ...
    'example_test.py',
    'mempool_truc.py',
    'wallet_multisig_descriptor_psmt.py',
    'wallet_miniscript_decaying_multisig_descriptor_psmt.py',
...
def main():
    ...
    enable_telestaid = config["components"].getboolean("ENABLE_BITCOIND")

    if not enable_telestaid:
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if telestaid is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "telestaid"], stdout=subprocess.DEVNULL).returncode == 0:
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Telestai functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/Telestai-Project/telestai/pull/27561.
