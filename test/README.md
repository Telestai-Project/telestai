This directory contains integration tests that test meowcoind and its
...
meowcoind and meowcoin-qt by interacting with them through the RPC and P2P
interfaces.
...
Before tests can be run locally, Meowcoin Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the meowcoind nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another meowcoind
process running on the system (perhaps from a previous test which hasn't successfully
killed all its meowcoind nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other meowcoind processes are running.

On linux, the test framework will warn if there is another
meowcoind process running when the tests are started.

If there are zombie meowcoind processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
meowcoind processes running on the system, so should not be used if any non-test
meowcoind processes are being run.**
...
sure meowcoind processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and meowcoind `debug.log`s will all be dumped
...
`test_framework.log` and meowcoind `debug.log`s can be combined into a single
...
call methods that interact with the meowcoind nodes-under-test.

If further introspection of the meowcoind instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/meowcoind.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/meowcoind <pid>
```
