Telestai Core version 0.18.0 is now available from:

  <https://telestaicore.org/bin/telestai-core-0.18.0/>
...
`/Applications/Telestai-Qt` (on Mac) or `telestaid`/`telestai-qt` (on
Linux).
...
recommended to use Telestai Core on unsupported systems.
...
10.10. Additionally, Telestai Core does not yet change appearance when
...
`contrib/init/telestaid.service`, it has been changed to use
`/var/lib/telestaid` as the data directory instead of
`~telestai/.telestai`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/telestaid` will
exist has enough space (check using `df -h /var/lib/telestaid`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/Telestai-Project/telestai/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://telestaicore.org/en/contact/) the Telestai Core
...
The systemd init file (`contrib/init/telestaid.service`) has been changed
to use `/var/lib/telestaid` as the data directory instead of
`~telestai/.telestai`. This change makes Telestai Core more consistent with
...
`datadir` option in `/etc/telestai/telestai.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/telestai/telestai.conf`.


Documentation
-------------

- A new short [document](https://github.com/Telestai-Project/telestai/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/Telestai-Project/telestai/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/Telestai-Project/telestai/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/Telestai-Project/telestai/blob/master/doc/telestai-conf.md)
  about the `telestai.conf` file describes how to use it to configure
  Telestai Core.

- A new document introduces Telestai Core's BIP174 [Partially-Signed
  Telestai Transactions
  (PSMT)](https://github.com/Telestai-Project/telestai/blob/master/doc/psmt.md)
...
- The [output script
  descriptor](https://github.com/Telestai-Project/telestai/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Telestai-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Telestai Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/Telestai-Project/telestai/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/Telestai-Project/telestai/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Telestai Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Telestai Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/Telestai-Project/telestai/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/Telestai-Project/telestai/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart telestaid with the `-deprecatedrpc=generate`
...
- A new `telestai-wallet` tool is now distributed alongside Telestai
...
This section describes planned changes to Telestai Core that may affect
other Telestai software and services.

- Since version 0.16.0, Telestai Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Telestai Core 0.20 (expected about
  a year after 0.18), Telestai Core will default to native segwit
  ...
  to bech32 addresses, and if the Telestai Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Telestai Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Telestai Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Telestai Core
...
- A sub-project of Telestai Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Telestai Core.  See their [project
  page](https://github.com/telestai-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Telestai Core's own implementation, although entropy
  gathered by Telestai Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Telestai Core a
...
- On macOS, Telestai Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `telestai_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libtelestai_server.a` (jnewbery)
...
- #13926 [Tools] telestai-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like telestaid (MarcoFalke)
...
- #14521 qt, docs: Fix `telestai-qt -version` output formatting (hebasto)
...
- #14204 Move `interfaces/*` to `libtelestai_server` (laanwj)
...
- #15154 Configure: telestai-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/telestai-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Telestai Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libtelestai_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in telestai-util-test.py (isghe)
...
- #12775 Integration of property based testing into Telestai Core (Christewart)
...
- #12246 Bugfix: Only run telestai-tx tests when telestai-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_telestai` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_telestai` (MarcoFalke)
...
- #14186 telestai-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_telestai.conf` message on startup if conf file exists (leishman)
...
- #12255 Update telestai.service to conform to init.md (dongcarl)
...
- #14497 Add doc/telestai-conf.md (hebasto)
...
- #15354 Add missing `telestai-wallet` tool manpages (MarcoFalke)
...
- #15765 correct telestaiconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support telestai folder on OSX (JimmyMow)
