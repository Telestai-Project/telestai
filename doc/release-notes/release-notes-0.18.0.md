Meowcoin Core version 0.18.0 is now available from:

  <https://meowcoincore.org/bin/meowcoin-core-0.18.0/>
...
`/Applications/Meowcoin-Qt` (on Mac) or `meowcoind`/`meowcoin-qt` (on
Linux).
...
recommended to use Meowcoin Core on unsupported systems.
...
10.10. Additionally, Meowcoin Core does not yet change appearance when
...
`contrib/init/meowcoind.service`, it has been changed to use
`/var/lib/meowcoind` as the data directory instead of
`~meowcoin/.meowcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/meowcoind` will
exist has enough space (check using `df -h /var/lib/meowcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/meowcoin/meowcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://meowcoincore.org/en/contact/) the Meowcoin Core
...
The systemd init file (`contrib/init/meowcoind.service`) has been changed
to use `/var/lib/meowcoind` as the data directory instead of
`~meowcoin/.meowcoin`. This change makes Meowcoin Core more consistent with
...
`datadir` option in `/etc/meowcoin/meowcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/meowcoin/meowcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/meowcoin/meowcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/meowcoin/meowcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/meowcoin/meowcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/meowcoin/meowcoin/blob/master/doc/meowcoin-conf.md)
  about the `meowcoin.conf` file describes how to use it to configure
  Meowcoin Core.
...
- The [output script
  descriptor](https://github.com/meowcoin/meowcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Meowcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Meowcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/meowcoin/meowcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/meowcoin/meowcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Meowcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Meowcoin Core, subsequently importing
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/meowcoin/meowcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart meowcoind with the `-deprecatedrpc=generate`
...
- A new `meowcoin-wallet` tool is now distributed alongside Meowcoin
...
This section describes planned changes to Meowcoin Core that may affect
other Meowcoin software and services.

- Since version 0.16.0, Meowcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Meowcoin Core 0.20 (expected about
  a year after 0.18), Meowcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Meowcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Meowcoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Meowcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Meowcoin Core
...
- A sub-project of Meowcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Meowcoin Core.  See their [project
  page](https://github.com/meowcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Meowcoin Core's own implementation, although entropy
  gathered by Meowcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Meowcoin Core a
...
- On macOS, Meowcoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `meowcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libmeowcoin_server.a` (jnewbery)
...
- #13926 [Tools] meowcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like meowcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `meowcoin-qt -version` output formatting (hebasto)
...
- #14204 Move `interfaces/*` to `libmeowcoin_server` (laanwj)
...
- #15154 Configure: meowcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/meowcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Meowcoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libmeowcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in meowcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Meowcoin Core (Christewart)
...
- #12246 Bugfix: Only run meowcoin-tx tests when meowcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_meowcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_meowcoin` (MarcoFalke)
...
- #14186 meowcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_meowcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update meowcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/meowcoin-conf.md (hebasto)
...
- #15354 Add missing `meowcoin-wallet` tool manpages (MarcoFalke)
