Telestai Core version 0.11.0 is now available from:

  <https://telestai.org/bin/telestai-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer TLS/kB fee to be
...
For example, add the following to `telestai.conf`:
...
related to the blockchain in the telestai system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Telestai Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Telestai Core will 
...
For an example script see [telestai-submittx](https://github.com/laanwj/telestai-submittx).
...
- #5334 `cf87536` libtelestaiconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new telestai-detached-sigs git repo for OSX signatures
...
- #5753 `0f44672` Add telestai logo to about screen
...
- #5809 `46bfbe7` Add telestai-cli man page
...
- #5849 `41259ca` contrib/init/telestaid.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Telestai-Qt.app to Telestai-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for telestai-tx)
