Meowcoin Core version 0.11.0 is now available from:

  <https://meowcoin.org/bin/meowcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer MEWC/kB fee to be
...
For example, add the following to `meowcoin.conf`:
...
related to the blockchain in the meowcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Meowcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Meowcoin Core will 
...
For an example script see [meowcoin-submittx](https://github.com/laanwj/meowcoin-submittx).
...
- #5334 `cf87536` libmeowcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new meowcoin-detached-sigs git repo for OSX signatures
...
- #5753 `0f44672` Add meowcoin logo to about screen
...
- #5809 `46bfbe7` Add meowcoin-cli man page
...
- #5849 `41259ca` contrib/init/meowcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Meowcoin-Qt.app to Meowcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for meowcoin-tx)
