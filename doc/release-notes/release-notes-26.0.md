...
Telestai Core version 26.0 is now available from:

  <https://telestaicore.org/bin/telestai-core-26.0/>
...
- Experimental support for the v2 transport protocol defined in
  [BIP324](https://github.com/telestai/bips/blob/master/bip-0324.mediawiki) was added.
...
- `finalizepsmt` is now able to finalize a PSMT with inputs spending [Miniscript](https://bitcoin.sipa.be/miniscript/)-compatible Taproot leaves. (#27255)
...
- `loadtxoutset` has been added, which allows loading a UTXO snapshot of the format
  ...
  document (<https://github.com/Telestai-Project/telestai/blob/master/doc/design/assumeutxo.md>).
...
- `telestaid` and `telestai-qt` will now raise an error on startup
 if a datadir that is being used contains a telestai.conf file that
 will be ignored, which can happen when a datadir= line is used in
 a telestai.conf file. The error message is just a diagnostic intended
 ...
 the telestai.conf contained in it. (#27302)
...
- A new `telestaiconsensus_verify_script_with_spent_outputs` function is available in libconsensus which optionally accepts the spent outputs of the transaction being verified.
- A new `telestaiconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT` flag is available in libconsensus that will verify scripts with the Taproot spending rules.
...
- Wallet loading has changed in this release. Wallets with some corrupted records that could be
  ...
  load the wallet in a previous version of Telestai Core and import the data into a new wallet.
...
```sh
src/telestai-cli -named bumpfee txid fee_rate=100
```

instead of

```sh
src/telestai-cli -named bumpfee txid options='{"fee_rate": 100}'
```
...
- The usage of hybrid public keys in output descriptors has been removed. Hybrid
  ...
  (as specified in BIP380 and documented in doc/descriptors.md). Telestai Core would
...
- Bash completion files have been renamed from `telestai*.bash-completion` to
  `telestai*.bash`. This means completions can be automatically loaded on demand
