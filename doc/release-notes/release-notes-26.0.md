...
Meowcoin Core version 26.0 is now available from:

  <https://meowcoincore.org/bin/meowcoin-core-26.0/>
...
- Experimental support for the v2 transport protocol defined in
  [BIP324](https://github.com/meowcoin/bips/blob/master/bip-0324.mediawiki) was added.
...
- `meowcoind` and `meowcoin-qt` will now raise an error on startup
 if a datadir that is being used contains a meowcoin.conf file that
 will be ignored, which can happen when a datadir= line is used in
 a meowcoin.conf file. The error message is just a diagnostic intended
 ...
 the meowcoin.conf contained in it. (#27302)
...
- A new `meowcoinconsensus_verify_script_with_spent_outputs` function is available in libconsensus which optionally accepts the spent outputs of the transaction being verified.
- A new `meowcoinconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT` flag is available in libconsensus that will verify scripts with the Taproot spending rules.
...
- Wallet loading has changed in this release. Wallets with some corrupted records that could be
  ...
  load the wallet in a previous version of Meowcoin Core and import the data into a new wallet.
...
```sh
src/meowcoin-cli -named bumpfee txid fee_rate=100
```

instead of

```sh
src/meowcoin-cli -named bumpfee txid options='{"fee_rate": 100}'
```
...
- It's now possible to use [Miniscript](https://meowcoin.sipa.be/miniscript/) inside Taproot leaves for descriptor wallets. (#27255)
...
- The usage of hybrid public keys in output descriptors has been removed. Hybrid
  ...
  (as specified in BIP380 and documented in doc/descriptors.md). Meowcoin Core would
...
- Bash completion files have been renamed from `meowcoin*.bash-completion` to
  `meowcoin*.bash`. This means completions can be automatically loaded on demand
