...
Meowcoin Core version 0.20.0 is now available from:

  <https://meowcoincore.org/bin/meowcoin-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Meowcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Meowcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Meowcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Meowcoin Core. Meowcoin Core logs debug messages
...
- OpenSSL is no longer used by Meowcoin Core.  (#17265)

- BIP70 support has been fully removed from Meowcoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/meowcoin/meowcoin/issues/16599),
  [PR #16702](https://github.com/meowcoin/meowcoin/pull/16702), and the
  `meowcoind help` for more information.  This option is experimental and
...
- All custom settings configured when Meowcoin Core starts are now
...
- The "Start Meowcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://meowcoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Meowcoin Core wallets
...
- Meowcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.meowcoincore.org.  (#17596)
...
- The `meowcoin-cli` utility used with the `-getinfo` parameter now
...
- The `meowcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17741 Included `test_meowcoin-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_meowcoin` binary (hebasto)
...
- #18702 Fix ASLR for meowcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate meowcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest meowcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_meowcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (meowcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone meowcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Meowcoin Core files (hebasto)
...
- #17119 Fix broken meowcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from meowcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.meowcoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to meowcoin-wallet (hel-o)
...
- #18341 Replace remaining literal MEWC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On meowcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from meowcoin-cli -getinfo (malevolent)
