...
Telestai Core version 0.20.0 is now available from:

  <https://telestaicore.org/bin/telestai-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Telestai Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Telestai Core version 0.18.0.
...
- Testing or debugging of implementations of the Telestai P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Telestai Core. Telestai Core logs debug messages
...
- OpenSSL is no longer used by Telestai Core.  (#17265)

- BIP70 support has been fully removed from Telestai Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/Telestai-Project/telestai/issues/16599),
  [PR #16702](https://github.com/Telestai-Project/telestai/pull/16702), and the
  `telestaid help` for more information.  This option is experimental and
...
- All custom settings configured when Telestai Core starts are now
...
- The "Start Telestai Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://telestaicore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Telestai Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Telestai Transaction (PSMT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/telestai-core/HWI) for
  signing.  Future versions of Telestai Core should support a GUI option
...
- Telestai Core's automatically-generated source code documentation is
  now available at https://doxygen.telestaicore.org.  (#17596)
...
- The `telestai-cli` utility used with the `-getinfo` parameter now
...
- The `telestai-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `telestaistrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_telestai-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_telestai` binary (hebasto)
...
- #18702 Fix ASLR for telestai-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate telestai schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest telestai.conf (sdaftuar)
...
- #18022 Fix appveyor `test_telestai` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (telestai-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone telestai-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Telestai Core files (hebasto)
...
- #17119 Fix broken telestai-cli examples (andrewtoth)
...
- #17503 Remove bitness from telestai-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.telestaicore.org (laanwj)
...
- #17648 Rename wallet-tool references to telestai-wallet (hel-o)
...
- #18341 Replace remaining literal TLS with `CURRENCY_UNIT` (domob1812)
...
- #16115 On telestaid startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from telestai-cli -getinfo (malevolent)
