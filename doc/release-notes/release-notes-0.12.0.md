Telestai Core version 0.12.0 is now available from:

  <https://telestai.org/bin/telestai-core-0.12.0/>
...
backwards-compatible with pre-0.10 versions of Telestai Core or other software:
...
pre-0.12 versions of Telestai Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Telestai Core version 0.11 or
earlier.
...
ECDSA signatures inside Telestai transactions now use validation using
[libsecp256k1](https://github.com/telestai-core/secp256k1) instead of OpenSSL.
...
(https://github.com/telestai/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of Telestai Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Telestai Core was to raise the default minimum
relay fee.

Telestai Core 0.12 will have a strict maximum size on the mempool. The
...
Telestai Core 0.12 also introduces new default policy limits on the length and
...
Telestai Core 0.12 nodes. Telestai Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/telestai/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in Telestai Core 0.12 does not yet have support for
...
This allows running telestaid without having to do any manual configuration.
...
Telestai Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Telestai Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Telestai Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Telestai Core 0.13, and it is at this time undecided whether the
...
Telestai Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Telestai Core automatically creates a hidden service to listen on, without
manual configuration. Telestai Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Telestai Core is listening, and
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Telestai
Core to use floating fees. By default, Telestai Core will use floating
fees.
...
(default: `0.0002` TLS/kB).

At all times, Telestai Core will cap fees at `-maxtxfee=<x>` (default:
0.10) TLS.
Furthermore, Telestai Core will never create transactions paying less than
...
`telestai.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/Telestai-Project/telestai/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/telestai/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/telestai/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /telestairpc>
        ...
        # optional bypass telestaid rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< telestairpc:<password>
    </Location>
...
Restarting telestaid will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Telestai-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new telestai-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include telestai-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix telestai-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of telestai-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [telestai-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libtelestaiconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "TLS" (Ross Nicoll)
