Telestai Core version 28.0 is now available from:

  <https://telestaicore.org/bin/telestai-core-28.0/>
...
Running Telestai Core binaries on macOS requires self signing.
```
cd /path/to/telestai-28.0/bin
xattr -d com.apple.quarantine telestai-cli telestai-qt telestai-tx telestai-util telestai-wallet telestaid test_telestai
codesign -s - telestai-cli telestai-qt telestai-tx telestai-util telestai-wallet telestaid test_telestai
```
...
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. Telestai
...
Support for Testnet4 as specified in [BIP94](https://github.com/telestai/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\Telestai`
to `C:\Users\Username\AppData\Local\Telestai`. Telestai Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/Telestai-Project/telestai/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libtelestaiconsensus Removal
---------------------------

The libtelestai-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if Telestai Core was listening for P2P connections, either using
...
- Telestai Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/telestai/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Telestai Core with the configuration
...
are now required to compile Telestai Core. (#29091, #30263)

- The minimum required glibc to run Telestai Core is now
...
- In RPC `bumpfee`, if a `fee_rate` is specified, the feerate is no longer restricted
  to following the wallet's incremental feerate of 5 tls/vb. The feerate must still be
...
- A new `-testdatadir` option has been added to `test_telestai` to allow specifying the
...
Previous releases of Telestai Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
