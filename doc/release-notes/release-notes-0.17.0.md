Telestai Core version 0.17.0 is now available from:

  <https://telestaicore.org/bin/telestai-core-0.17.0/>
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `telestai.conf` file, not inside included files or from
  ...
  as telestai.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the telestai.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=telestai
    test.uacomment=telestai-testnet
...
be used in V0.17 by starting telestaid with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Telestai Transactions support
-----------------------------------------------------

[BIP 174 PSMT](https://github.com/telestai/bips/blob/master/bip-0174.mediawiki) is an interchange format for Telestai transactions that are not fully signed
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in TLS. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When telestai is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If telestai is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart telestaid with
...
- When running telestaid with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If telestaid
...
- #11962 `0a01843` add seed.telestai.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in TLS (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI telestai:// (krab)
...
- #13486 `66e1a08` Move rpc/util.cpp from libtelestai-util to libtelestai-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for telestai-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling telestai-qt (TheCharlatan)
...
- #12443 `07090c5` Move common args to telestai.conf (MarcoFalke)
...
- #12882 `d96bdd7` Make `test_telestai` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.telestai is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_telestai` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in telestaid systemd service file (Flowdalic)
- #12652 `55f490a` telestai-cli: Provide a better error message when telestaid is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to telestaid in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for telestai address in translations (kaplanmaxe)
...
- #13603 `453ae5e` telestai-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix telestai-cli --version (Empact)
...
- #12877 `2b54155` Use telestaid in Tor documentation (knoxcard)
...
- #13346 `60f0358` update telestai-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust telestaicore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed telestai-cli -help output for help2man (hebasto)
