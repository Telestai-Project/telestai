...
* Update meowcoin.conf and commit changes if they exist, see [gen-meowcoin-conf.sh](/contrib/devtools/README.md#gen-meowcoin-confsh).
...
* On the new release branch in [`CMakeLists.txt`](../CMakeLists.txt)(see [this commit](https://github.com/meowcoin/meowcoin/commit/742f7dd)):
...
* Update hardcoded [seeds](/contrib/seeds/README.md), see [this pull request](https://github.com/meowcoin/meowcoin/pull/27488) for an example.
* Update the following variables in [`src/kernel/chainparams.cpp`](/src/kernel/chainparams.cpp) for mainnet, testnet, and signet:
  ...
  - `chainTxData` with statistics about the transaction count and rate. Use the output of the `getchaintxstats` RPC with an
    ...
    [this pull request](https://github.com/meowcoin/meowcoin/pull/28591) for an example. Reviewers can verify the results by running
  ...
  - `m_assumeutxo_data` array should be appended to with the values returned by calling `meowcoin-cli -rpcclienttimeout=0 -named dumptxoutset utxo.dat rollback=<height or hash>`
...
- Translations on Transifex:
    ...
    - Create [a new resource](https://app.transifex.com/meowcoin/meowcoin/content/) named after the major version with the slug `qt-translation-<RRR>x`, where `RRR` is the major branch number padded with zeros. Use `src/qt/locale/meowcoin_en.xlf` to create it.
    ...
    - Make an announcement that translators can start translating for the new version. You can use one of the [previous announcements](https://app.transifex.com/meowcoin/communication/) as a template.
    - Change the auto-update URL for the resource to `master`, e.g. `https://raw.githubusercontent.com/meowcoin/meowcoin/master/src/qt/locale/meowcoin_en.xlf`. (Do this only after the previous steps, to prevent an auto-update from interfering.)

#### After branch-off (on the major release branch)

- Update the versions.
- Create the draft, named "*version* Release Notes Draft", as a [collaborative wiki](https://github.com/meowcoin-core/meowcoin-devwiki/wiki/_new).
- Clear the release notes: `cp doc/release-notes-empty-template.md doc/release-notes.md`
- Create a pinned meta-issue for testing the release candidate (see [this issue](https://github.com/meowcoin/meowcoin/issues/27621) for an example) and provide a link to it in the release announcements where useful.
- Translations on Transifex
    - Change the auto-update URL for the new major version's resource away from `master` and to the branch, e.g. `https://raw.githubusercontent.com/meowcoin/meowcoin/<branch>/src/qt/locale/meowcoin_en.xlf`. Do not forget this or it will keep tracking the translations on master instead, drifting away from the specific major release.
- Prune inputs from the qa-assets repo (See [pruning
  inputs](https://github.com/meowcoin-core/qa-assets#pruning-inputs)).

#### Before final release

- Merge the release notes from [the wiki](https://github.com/meowcoin-core/meowcoin-devwiki/wiki/) into the branch.
- Ensure the "Needs release note" label is removed from all relevant pull
  requests and issues:
  https://github.com/meowcoin/meowcoin/issues?q=label%3A%22Needs+release+note%22

#### Tagging a release (candidate)

To tag the version (or release candidate) in git, use the `make-tag.py` script from [meowcoin-maintainer-tools](https://github.com/meowcoin-core/meowcoin-maintainer-tools). From the root of the repository run:

    ../meowcoin-maintainer-tools/make-tag.py v(new version, e.g. 25.0)
...
Check out the source code in the following directory hierarchy.
    ...
    git clone https://github.com/meowcoin-core/guix.sigs.git
    git clone https://github.com/meowcoin-core/meowcoin-detached-sigs.git
    git clone https://github.com/meowcoin/meowcoin.git

### Write the release notes

Open a draft of the release notes for collaborative editing at https://github.com/meowcoin-core/meowcoin-devwiki/wiki.
...
Checkout the Meowcoin Core version you'd like to build:
...
Then open a Pull Request to the [guix.sigs repository](https://github.com/meowcoin-core/guix.sigs).
...
In the `guix-build-${VERSION}/output/x86_64-apple-darwin` and `guix-build-${VERSION}/output/arm64-apple-darwin` directories:
    ...
    tar xf meowcoin-${VERSION}-${ARCH}-apple-darwin-codesigning.tar.gz
...
In the `guix-build-${VERSION}/output/x86_64-w64-mingw32` directory:
    ...
    tar xf meowcoin-${VERSION}-win64-codesigning.tar.gz
...
However if this is done, once the release has been tagged in the meowcoin-detached-sigs repo, the `guix-codesign` step must be performed again in order for the guix attestation to be valid when compared against the attestations of non-codesigner builds. The directories created by `guix-codesign` will need to be cleared prior to running `guix-codesign` again.

### Windows and macOS codesigners only: Commit the detached codesign payloads

```sh
pushd ./meowcoin-detached-sigs
...
- Upload to the meowcoincore.org server:
    1. The contents of each `./meowcoin/guix-build-${VERSION}/output/${HOST}/` directory.
...
- After uploading release candidate binaries, notify the meowcoin-core-dev mailing list and
  meowcoin-dev group that a release candidate is available for testing. Include a link to the release
  notes draft.
...
- Optionally help seed this torrent. To get the `magnet:` URI use:
  ...
  people without access to `meowcoincore.org` to download the binary distribution.
...
- Update the meowcoincore.org website

  - blog post

  - maintained versions [table](https://github.com/meowcoin-core/meowcoincore.org/commits/master/_includes/posts/maintenance-table.md)

  - RPC documentation update

      - See https://github.com/meowcoin-core/meowcoincore.org/blob/master/contrib/doc-gen/


- Update repositories

  - Delete post-EOL [release branches](https://github.com/meowcoin/meowcoin/branches/all) and create a tag `v${branch_name}-final`.

  - Delete ["Needs backport" labels](https://github.com/meowcoin/meowcoin/labels?q=backport) for non-existing branches.

  - Update packaging repo

      - Push the flatpak to flathub, e.g. https://github.com/flathub/org.meowcoincore.meowcoin-qt/pull/2

      - Push the snap, see https://github.com/meowcoin-core/packaging/blob/main/snap/local/build.md

  - Create a [new GitHub release](https://github.com/meowcoin/meowcoin/releases/new) with a link to the archived release notes

- Announce the release:

  - meowcoin-dev and meowcoin-core-dev mailing list

  - Meowcoin Core announcements list https://meowcoincore.org/en/list/announcements/join/

  - Meowcoin Core Twitter https://twitter.com/meowcoincoreorg
