...
Changes to the configuration file while `telestaid` or `telestai-qt` is running only take effect after restarting.
...
The `settings.json` file contains dynamic settings that are set by the Telestai Core GUI and RPCs at runtime, and augment or replace the static settings specified in the `telestai.conf` file.

Command line options also augment or replace `telestai.conf` options, and can be useful for scripting and debugging.

It is possible to see which setting values are in use by checking `debug.log` output. Any unrecognized options that are found in `telestai.conf` also show up as warnings in `debug.log` output.
...
-- | -- | --
Windows | `%LOCALAPPDATA%\Telestai\` | `C:\Users\username\AppData\Local\Telestai\telestai.conf`
Linux | `$HOME/.telestai/` | `/home/username/.telestai/telestai.conf`
macOS | `$HOME/Library/Application Support/Telestai/` | `/Users/username/Library/Application Support/Telestai/telestai.conf`

An example configuration file can be generated by [contrib/devtools/gen-telestai-conf.sh](../contrib/devtools/gen-telestai-conf.sh).
Run this script after compiling to generate an up-to-date configuration file.
The output is placed under `share/examples/telestai.conf`.
To use the generated configuration file, copy the example file into your data directory and edit it there, like so:

```
# example copy command for linux user
cp share/examples/telestai.conf ~/.telestai
```
