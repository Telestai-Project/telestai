...
Changes to the configuration file while `meowcoind` or `meowcoin-qt` is running only take effect after restarting.
...
The `settings.json` file contains dynamic settings that are set by the Meowcoin Core GUI and RPCs at runtime, and augment or replace the static settings specified in the `meowcoin.conf` file.

Command line options also augment or replace `meowcoin.conf` options, and can be useful for scripting and debugging.

It is possible to see which setting values are in use by checking `debug.log` output. Any unrecognized options that are found in `meowcoin.conf` also show up as warnings in `debug.log` output.
...
-- | -- | --
Windows | `%LOCALAPPDATA%\Meowcoin\` | `C:\Users\username\AppData\Local\Meowcoin\meowcoin.conf`
Linux | `$HOME/.meowcoin/` | `/home/username/.meowcoin/meowcoin.conf`
macOS | `$HOME/Library/Application Support/Meowcoin/` | `/Users/username/Library/Application Support/Meowcoin/meowcoin.conf`

An example configuration file can be generated by [contrib/devtools/gen-meowcoin-conf.sh](../contrib/devtools/gen-meowcoin-conf.sh).
Run this script after compiling to generate an up-to-date configuration file.
The output is placed under `share/examples/meowcoin.conf`.
To use the generated configuration file, copy the example file into your data directory and edit it there, like so:

```
# example copy command for linux user
cp share/examples/meowcoin.conf ~/.meowcoin
```
