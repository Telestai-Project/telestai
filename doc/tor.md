# TOR SUPPORT IN TELESTAI

It is possible to run Telestai Core as a Tor onion service, and connect to such services.
...
- Starting with version 22.0, Telestai Core only supports Tor version 3 hidden
  services (Tor v3). Tor v2 addresses are ignored by Telestai Core and neither
  relayed nor stored.

- Tor removed v2 support beginning with version 0.4.6.

## How to see information about your Tor configuration via Telestai Core

There are several ways to see your local onion address in Telestai Core:
...
You can use the `getnodeaddresses` RPC to fetch a number of onion peers known to your node; run `telestai-cli help getnodeaddresses` for details.

`telestai rpc` can also be substituted for `telestai-cli`.

## 1. Run Telestai Core behind a Tor proxy

The first step is running Telestai Core behind a Tor proxy. This will already anonymize all
...
In a typical situation, this suffices to run behind a Tor proxy:
    ...
    telestaid -proxy=127.0.0.1:9050

`telestai node` or `telestai gui` can also be substituted for `telestaid`.

## 2. Automatically create a Telestai Core onion service

Telestai Core makes use of Tor's control socket API to create and destroy
ephemeral onion services programmatically. This means that if Tor is running and
proper authentication has been configured, Telestai Core automatically creates an
onion service to listen on. The goal is to increase the number of available
onion nodes.

This feature is enabled by default if Telestai Core is listening (`-listen`) and
it requires a Tor connection to work. It can be explicitly disabled with
`-listenonion=0`. If it is not disabled, it can be configured using the
`-torcontrol` and `-torpassword` settings.

To see verbose Tor information in the telestaid debug log, pass `-debug=tor`.
...
methods to be configured: cookie authentication or telestaid's `-torpassword`
configuration option.

#### Cookie authentication

For cookie authentication, the user running telestaid must have read access to
the `CookieAuthFile` specified in the Tor configuration. In some cases this is
preconfigured and the creation of an onion service is automatic. Don't forget to
use the `-debug=tor` telestaid configuration option to enable Tor debug logging.

If a permissions problem is seen in the debug log, e.g. `tor: Authentication
cookie /run/tor/control.authcookie could not be opened (check permissions)`, it
can be resolved by adding both the user running Tor and the user running
telestaid to the same Tor group and setting permissions appropriately.
...
run telestaid, run this as root:
...
## 3. Manually create a Telestai Core onion service

You can also manually configure your node to be reachable from the Tor network.
Add these lines to your `/etc/tor/torrc` (or equivalent config file):

    HiddenServiceDir /var/lib/tor/telestai-service/
    HiddenServicePort 8333 127.0.0.1:8334

The directory can be different of course, but virtual port numbers should be equal to
your telestaid's P2P listen port (8333 by default), and target addresses and ports
should be equal to binding address and port for inbound Tor connections (127.0.0.1:8334 by default).

    -externalip=X   You can tell telestai about its publicly reachable addresses using
                    this option, and this can be an onion address. Given the above
                    configuration, you can find your onion address in
                    /var/lib/tor/telestai-service/hostname. For connections
...
In a typical situation, where you're only reachable via Tor, this should suffice:
    ...
    telestaid -proxy=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -listen

(obviously, replace the .onion address with your own). It should be noted that you still
listen on all devices and another node could establish a clearnet connection, when knowing
your address. To mitigate this, additionally bind the address of your Tor proxy:

    telestaid ... -bind=127.0.0.1:8334=onion
...
for normal IPv4/IPv6 communication, use:
    ...
    telestaid -onion=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -discover

## 4. Privacy recommendations

- Do not add anything but Telestai Core ports to the onion service created in section 3.
