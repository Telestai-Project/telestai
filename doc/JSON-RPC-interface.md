...
This is the endpoint used by telestai-cli when a `-rpcwallet=` parameter is passed in.
...
telestai-cli createwallet mywallet false false "" false false true

# "params": {"wallet_name": "mywallet", "load_on_startup": true}
telestai-cli -named createwallet wallet_name=mywallet load_on_startup=true

# "params": {"args": ["mywallet"], "load_on_startup": true}
telestai-cli -named createwallet mywallet load_on_startup=true
```

`telestai rpc` can also be substituted for `telestai-cli -named`, and is a newer alternative.
...
- **Secure authentication:** By default, when no `rpcpassword` is specified, Telestai Core generates unique
