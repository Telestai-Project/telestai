...
Currently, it is possible to create a multisig wallet using Telestai Core only.

Although there is already a brief explanation about the multisig in the [Descriptors documentation](https://github.com/Telestai-Project/telestai/blob/master/doc/descriptors.md#multisig), this tutorial proposes to use the signet (instead of regtest), bringing the reader closer to a real environment and explaining some functions in more detail.
...
Before starting this tutorial, start the telestai node on the signet network.

```bash
./build/bin/telestaid -signet -daemon
```

This tutorial also uses the default WPKH derivation path to get the xpubs and does not conform to [BIP 45](https://github.com/telestai/bips/blob/master/bip-0045.mediawiki) or [BIP 87](https://github.com/telestai/bips/blob/master/bip-0087.mediawiki).

At the time of writing, there is no way to extract a specific path from wallets in Telestai Core. For this, an external signer/xpub can be used.
...
for ((n=1;n<=3;n++))
do
 ./build/bin/telestai-cli -signet createwallet "participant_${n}"
done
```

`telestai rpc` can also be substituted for `telestai-cli`.

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, Telestai Core single-sig wallets are created using path `m/44'/1'/0'` for PKH, `m/84'/1'/0'` for WPKH, `m/49'/1'/0'` for P2WPKH-nested-in-P2SH and `m/86'/1'/0'` for P2TR based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
All RPCs in Telestai Core will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./build/bin/telestai-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./build/bin/telestai-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
As previously mentioned, this step extracts the `m/84'/1'/0'` account instead of the path defined in [BIP 45](https://github.com/telestai/bips/blob/master/bip-0045.mediawiki) or [BIP 87](https://github.com/telestai/bips/blob/master/bip-0087.mediawiki), since there is no way to extract a specific path in Telestai Core at the time of writing.
...
external_desc_sum=$(./build/bin/telestai-cli -signet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./build/bin/telestai-cli -signet getdescriptorinfo $internal_desc | jq '.descriptor')
...
`external_desc` and `internal_desc` specify the output type (`wsh`, in this case) and the xpubs involved. They also use BIP 67 (`sortedmulti`), so the wallet can be recreated without worrying about the order of xpubs. Conceptually, descriptors describe a list of scriptPubKey (along with information for spending from it) [[source](https://github.com/Telestai-Project/telestai/issues/21199#issuecomment-780772418)].

Note that at least two descriptors are usually used, one for internal derivation paths and one for external ones. There are discussions about eliminating this redundancy, as can be seen in the issue [#17190](https://github.com/Telestai-Project/telestai/issues/17190).
...
Documentation for these and other parameters can be found by typing `./build/bin/telestai-cli help importdescriptors`.
...
```bash
./build/bin/telestai-cli -signet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/telestai-cli  -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/telestai-cli  -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/telestai-cli -signet loadwallet "participant_${n}"; done
```
...
```bash
receiving_address=$(./build/bin/telestai-cli -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./build/bin/telestai-cli -a $receiving_address
```
...
```bash
./build/bin/telestai-cli -signet -rpcwallet="multisig_wallet_01" getbalances
```

### 1.5 Create a PSMT

Unlike singlesig wallets, multisig wallets cannot create and sign transactions directly because they require the signatures of the co-signers. Instead they create a Partially Signed Telestai Transaction (PSMT).

PSMT is a data format that allows wallets and other tools to exchange information about a Telestai transaction and the signatures necessary to complete it. [[source](https://telestaiops.org/en/topics/psmt/)]

The current PSMT version (v0) is defined in [BIP 174](https://github.com/telestai/bips/blob/master/bip-0174.mediawiki).

For simplicity, the destination address is taken from the `participant_1` wallet in the code above, but it can be any valid telestai address.
...
```bash
balance=$(./build/bin/telestai-cli -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/telestai-cli -signet -rpcwallet="participant_1" getnewaddress)

funded_psmt=$(./build/bin/telestai-cli -signet -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsmt outputs="{\"$destination_addr\": $amount}" | jq -r '.psmt')
```

There is also the `createpsmt` RPC, which serves the same purpose, but it has no access to the wallet or to the UTXO set. It is functionally the same as `createrawtransaction` and just drops the raw transaction into an otherwise blank PSMT. [[source](https://telestaitalk.org/index.php?topic=5131043.msg50573609#msg50573609)] In most cases, `walletcreatefundedpsmt` solves the problem.
...
```bash
./build/bin/telestai-cli -signet decodepsmt $funded_psmt

./build/bin/telestai-cli -signet analyzepsmt $funded_psmt
```
...
```bash
psmt_1=$(./build/bin/telestai-cli -signet -rpcwallet="participant_1" walletprocesspsmt $funded_psmt | jq '.psmt')

psmt_2=$(./build/bin/telestai-cli -signet -rpcwallet="participant_2" walletprocesspsmt $funded_psmt | jq '.psmt')
```
...
```bash
combined_psmt=$(./build/bin/telestai-cli -signet combinepsmt "[$psmt_1, $psmt_2]")
```
...
```bash
finalized_psmt_hex=$(./build/bin/telestai-cli -signet finalizepsmt $combined_psmt | jq -r '.hex')

./build/bin/telestai-cli -signet sendrawtransaction $finalized_psmt_hex
```
...
```bash
psmt_1=$(./build/bin/telestai-cli -signet -rpcwallet="participant_1" walletprocesspsmt $funded_psmt | jq -r '.psmt')

psmt_2=$(./build/bin/telestai-cli -signet -rpcwallet="participant_2" walletprocesspsmt $psmt_1 | jq -r '.psmt')

finalized_psmt_hex=$(./build/bin/telestai-cli -signet finalizepsmt $psmt_2 | jq -r '.hex')
