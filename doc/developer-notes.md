...
The Telestai Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to run with "play telestais" on the test network, if you
...
Telestai Core is a multithreaded application, and deadlocks or other multithreading bugs
...
However, in open source software such as Telestai Core, where everyone uses
...
be sent upstream but bugfixes may also be prudent to PR against Telestai so that they can be integrated
