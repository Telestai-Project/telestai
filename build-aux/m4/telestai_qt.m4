...
dnl Copyright (c) 2021 The Telestai Core developers
...
AC_DEFUN([TELESTAI_QT_FAIL],[
  if test "x$telestai_qt_want_version" = xauto && test "x$telestai_qt_force" != xyes; then
    ...
    telestai_enable_qt=no
    telestai_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([TELESTAI_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build telestai-qt GUI (default=auto)])],
    [
     telestai_qt_want_version=$withval
     if test "x$telestai_qt_want_version" = xyes; then
       telestai_qt_force=yes
       telestai_qt_want_version=auto
     fi
    ],
    [telestai_qt_want_version=auto])
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); telestai_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
AC_DEFUN([TELESTAI_QT_CONFIGURE],[
  ...
  TELESTAI_QT_CHECK([
    ...
    telestai_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      telestai_enable_qt_dbus=yes
    fi
  ...
  if test x$telestai_enable_qt = xyes; then
    AC_MSG_RESULT([$telestai_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$telestai_enable_qt])
  fi
...
AC_DEFUN([_TELESTAI_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, telestai_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      ...
      [telestai_cv_static_qt=yes],
      [telestai_cv_static_qt=no])
    ])
])
