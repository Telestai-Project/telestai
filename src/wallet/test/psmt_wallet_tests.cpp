...
namespace wallet {
BOOST_FIXTURE_TEST_SUITE(psmt_wallet_tests, WalletTestingSetup)
...
BOOST_AUTO_TEST_CASE(psmt_updater_test)
{
    ...
    bool complete = true;
    BOOST_REQUIRE(!m_wallet.FillPSMT(psmtx, complete, std::nullopt, false, true));
    ...
    psmtx.tx->vin[0].prevout.n = 2;

    // Try to sign the mutated input
    SignatureData sigdata;
    BOOST_CHECK(m_wallet.FillPSMT(psmtx, complete, std::nullopt, true, true));
}
