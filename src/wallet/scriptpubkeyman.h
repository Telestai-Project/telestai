...
public:
    ...
    /** Adds script and derivation path information to a PSMT, and optionally signs it. */
    virtual std::optional<common::PSMTError> FillPSMT(PartiallySignedTransaction& psmt, const PrecomputedTransactionData& txdata, std::optional<int> sighash_type = std::nullopt, bool sign = true, bool bip32derivs = false, int* n_signed = nullptr, bool finalize = true) const { return common::PSMTError::UNSUPPORTED; }
...
public:
    ...
    //! Load a PQ key from wallet storage (used by LoadWallet), without saving to disk.
    bool LoadPQKey(CPQKey&& key);
    //! Generate a new ML-DSA-44 key, persist it, and return the witness v2 destination.
...
public:
    ...
    std::optional<common::PSMTError> FillPSMT(PartiallySignedTransaction& psmt, const PrecomputedTransactionData& txdata, std::optional<int> sighash_type = std::nullopt, bool sign = true, bool bip32derivs = false, int* n_signed = nullptr, bool finalize = true) const override;
