...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given telestai address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The telestai address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The telestai address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Telestai address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are telestai addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) telestai address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The telestai address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CTelestaiAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CTelestaiAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
{
    if (params[0].isStr()) {
        CTelestaiAddress address(params[0].get_str());
        uint160 hashBytes;
        int type = 0;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CTelestaiAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    ...
    if (includeAssets) {
        ...
        if (!mempool.getAddressIndex(addresses, TLS, indexes)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
        }
    }
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    ...
    if (includeAssets) {
        ...
        for (std::vector<std::pair<uint160, int> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
            if (!GetAddressIndex((*it).first, (*it).second, TLS, addressIndex)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
            }
        }
...
UniValue getaddresstxids(const JSONRPCRequest& request)
{
    ...
    for (std::vector<std::pair<uint160, int> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
        if (includeAssets) {
            ...
            if (start > 0 && end > 0) {
                if (!GetAddressIndex((*it).first, (*it).second, TLS, addressIndex, start, end)) {
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
                }
            } else {
                if (!GetAddressIndex((*it).first, (*it).second, TLS, addressIndex)) {
