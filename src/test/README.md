...
To run the telestaid tests manually, launch `src/test/test_telestai`. To recompile
after a test file was modified, run `make` and then run the test again. If you
modify a non-test file, use `make -C src/test` to recompile only what's needed
to run the telestaid tests.

To add more telestaid tests, add `BOOST_AUTO_TEST_CASE` functions to the existing
.cpp files in the `test/` directory or add new .cpp files that
implement new BOOST_AUTO_TEST_SUITE sections.

To run the telestai-qt tests manually, launch `src/qt/test/test_telestai-qt`
...
the following from 'false' to 'true' in the `test_telestai.cpp` file and uncomment
three lines in the `script\interpreter.cpp\ VerifyScript` method and recompile:

    src\test\test_telestai.cpp:
...
Run `test_telestai --list_content` to get a full list of available unit tests.

To run just the 'getarg_tests' (verbosely):

    test_telestai --run_test=getarg_tests

... or to run just the doubledash test:

    test_telestai --run_test=getarg_tests/doubledash_test
...
The build system is setup to compile an executable called `test_telestai`
that runs all of the unit tests.  The main source file is called
test_telestai.cpp. To add a new unit test file to our test suite you need 
