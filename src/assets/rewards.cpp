...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for TLS
    ...
    if (p_rewardSnapshot.strDistributionAsset != "TLS") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is TLS with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using TLS differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "TLS") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Telestai address (already validated during ownership snapshot creation)
