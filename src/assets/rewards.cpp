...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  // Default to divisor for native TLS
    ...
    if (p_rewardSnapshot.strDistributionAsset != NATIVE_ASSET_TICKER) {
        ...
        LogPrintf("%s: Distribution is TLS with divisor %d\n", __func__, srcUnitDivisor);
    }
