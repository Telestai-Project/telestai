...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for MEWC
    ...
    if (p_rewardSnapshot.strDistributionAsset != "MEWC") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is MEWC with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using MEWC differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "MEWC") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Meowcoin address (already validated during ownership snapshot creation)
