...
Options:
  -m, --multiprocess     Run multiprocess binaries meowcoin-node, meowcoin-gui.
  -M, --monolithic       Run monolithic binaries meowcoind, meowcoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'meowcoin-qt [ARGS]' or 'meowcoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'meowcoind [ARGS]' or 'meowcoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'meowcoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'meowcoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'meowcoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_meowcoin [ARGS]'.
  chainstate [ARGS] Run meowcoin kernel chainstate util, equivalent to running 'meowcoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_meowcoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_meowcoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "meowcoin-gui" : "meowcoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "meowcoin-node" : "meowcoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("meowcoin-cli");
            // Since "meowcoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("meowcoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("meowcoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_meowcoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("meowcoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_meowcoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_meowcoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("meowcoin-util");
        } else {
...
//! Execute the specified meowcoind, meowcoin-qt or other command line in `args`
...
//!             be a relative path to a meowcoind, meowcoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/meowcoin ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/meowcoin/meowcoin/pull/31375#discussion_r1861814807)
