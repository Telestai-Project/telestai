...
void TestGUIWatchOnly(interfaces::Node& node, TestChain100Setup& test)
{
    ...
    // Time to reject "save" PSMT dialog ('SendCoins' locks the main thread until the dialog receives the event).
    QTimer timer;
    timer.setInterval(500);
    QObject::connect(&timer, &QTimer::timeout, [&](){
        for (QWidget* widget : QApplication::topLevelWidgets()) {
            if (widget->inherits("QMessageBox") && widget->objectName().compare("psmt_copied_message") == 0) {
    ...
    // Send tx and verify PSMT copied to the clipboard.
    SendCoins(*wallet.get(), sendCoinsDialog, PKHash(), 5 * COIN, /*rbf=*/false, QMessageBox::Save);
    const std::string& psmt_string = QApplication::clipboard()->text().toStdString();
    QVERIFY(!psmt_string.empty());

    // Decode psmt
    std::optional<std::vector<unsigned char>> decoded_psmt = DecodeBase64(psmt_string);
    QVERIFY(decoded_psmt);
    PartiallySignedTransaction psmt;
    std::string err;
    QVERIFY(DecodeRawPSMT(psmt, MakeByteSpan(*decoded_psmt), err));
}
