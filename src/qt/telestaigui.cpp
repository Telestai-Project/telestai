...
void TelestaiGUI::createActions()
{
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to TLS addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    openAction->setStatusTip(tr("Open a telestai: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Telestai command-line options").arg(tr(PACKAGE_NAME)));
...
void TelestaiGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Telestai network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void TelestaiGUI::checkAssets()
{
    ...
    if(AreAssetsDeployed()) {
        ...
        transferAssetAction->setToolTip(tr("Transfer assets to TLS addresses"));
