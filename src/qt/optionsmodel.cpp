...
/** Call node.updateRwSetting() with Meowcoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Meowcoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/meowcoin/meowcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because meowcoin 22.x will not try to read these.
...
//! Convert enabled/size values to meowcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from meowcoin -prune setting.
...
//! Get pruning size value to show in GUI from meowcoin -prune setting. If
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if meowcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_meowcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BitcoinUnit>() == m_display_meowcoin_unit) return;
    m_display_meowcoin_unit = new_unit.value<BitcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayBitcoinUnit", QVariant::fromValue(m_display_meowcoin_unit));
    Q_EMIT displayUnitChanged(m_display_meowcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/meowcoin/meowcoin/pull/8273
    ...
    // (https://github.com/meowcoin-core/gui/issues/567).
