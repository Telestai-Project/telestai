...
/** Call node.updateRwSetting() with Telestai 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Telestai 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/Telestai-Project/telestai/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because telestai 22.x will not try to read these.
...
//! Convert enabled/size values to telestai -prune setting.
...
//! Get pruning enabled value to show in GUI from telestai -prune setting.
...
//! Get pruning size value to show in GUI from telestai -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    if (unit.canConvert<BitcoinUnit>()) {
        ...
        m_display_unit = BitcoinUnit::TLS;
        settings.setValue("DisplayBitcoinUnit", QVariant::fromValue(m_display_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if telestai.conf value
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/Telestai-Project/telestai/pull/8273
    ...
    // (https://github.com/telestai-core/gui/issues/567).
