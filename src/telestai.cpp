...
Options:
  -m, --multiprocess     Run multiprocess binaries telestai-node, telestai-gui.
  -M, --monolithic       Run monolithic binaries telestaid, telestai-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'telestai-qt [ARGS]' or 'telestai-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'telestaid [ARGS]' or 'telestai-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'telestai-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'telestai-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'telestai-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_telestai [ARGS]'.
  chainstate [ARGS] Run telestai kernel chainstate util, equivalent to running 'telestai-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_telestai [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_telestai-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "telestai-gui" : "telestai-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "telestai-node" : "telestaid");
        } else if (cmd.command == "rpc") {
            args.emplace_back("telestai-cli");
            // Since "telestai rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("telestai-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("telestai-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_telestai");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("telestai-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_telestai");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_telestai-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("telestai-util");
        } else {
...
//! Execute the specified telestaid, telestai-qt or other command line in `args`
...
//!             be a relative path to a telestaid, telestai-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/telestai ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/Telestai-Project/telestai/pull/31375#discussion_r1861814807)
