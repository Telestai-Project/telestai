...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " telestai-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The telestai-tx tool is used for creating and modifying telestai transactions.\n\n"
                "telestai-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded telestai transaction, or with \"-create [commands]\" to create a hex-encoded telestai transaction.\n"
                "\n"
                "Usage: telestai-tx [options] <hex-tx> [commands]\n"
                "or:    telestai-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded telestai transaction
