...
"""Script for verifying Telestai Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from telestaicore.org and telestai.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/telestai-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://telestaicore.org"
HOST2 = "https://telestai.org"
VERSIONPREFIX = "telestai-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Telestai community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"telestai_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by telestaicore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the telestai release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes telestai.org lags behind telestaicore.org.)')
    )
