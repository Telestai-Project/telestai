...
"""Script for verifying Meowcoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from meowcoincore.org and meowcoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/meowcoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://meowcoincore.org"
HOST2 = "https://meowcoin.org"
VERSIONPREFIX = "meowcoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Meowcoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"meowcoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by meowcoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the meowcoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes meowcoin.org lags behind meowcoincore.org.)')
    )
