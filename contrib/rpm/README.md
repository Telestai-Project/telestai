...
The RPM spec file provided here is for Telestai-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://telestai.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Telestai git repository](https://github.com/TelestaiProject/Telestai) but are not part of
...
Currently only `Source10` is used, for the example `telestai.conf` file.
...
in the telestai git repository. At present only an SVG version of the Telestai
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the telestai
...
needed when 0.12.1 is released, a proper fix is already in the Telestai git
master branch.
...
idea to build Telestai Core against a static version of that library compiled
...
The `telestai.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `telestai-server` package. At this point, most RPM based
...
The `telestai-server` package should have SELinux support. How to properly do
...
Telestai-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/telestai/). He has been
packaging Telestai for Fedora at least since 2012.
...
4. I renamed the `telestai` package that contains the Qt GUI to `telestai-core` as
that appears to be how the general population refers to it, in contrast to
`telestai-xt` or `telestai-classic`. I wanted to make sure the general population
