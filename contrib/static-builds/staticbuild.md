...
sudo find ${DISTNAME}/bin -type f -executable -exec ../telestai/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../telestai/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/telestai-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer telestai-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer telestai-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
#transfer telestai-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
~/telestai/contrib/macdeploy/extract-osx-sdk.sh
...
git clone https://github.com/telestaiproject/telestai
mkdir ~/telestai/depends/SDKs
#transfer MacOSX10.11.sdk.tar.gz to the folder telestai/depends/SDKs
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/telestai-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer telestai-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
