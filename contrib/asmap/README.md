...
be used in Meowcoin Core with the `-asmap` option.
...
to binary before being used with Meowcoin Core.
...
the `meowcoin-cli getnodeaddresses` command.
...
```
meowcoin-cli getnodeaddresses 0 > addrs.json
```
