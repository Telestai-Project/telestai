...
be used in Telestai Core with the `-asmap` option.
...
to binary before being used with Telestai Core.
...
the `telestai-cli getnodeaddresses` command.
...
```
telestai-cli getnodeaddresses 0 > addrs.json
```
