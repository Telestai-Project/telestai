...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing meowcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='meowcoin-cli', help='meowcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Meowcoin address to which the faucet should send')
...
parser.add_argument('meowcoin_cli_args', nargs='*', help='Arguments to pass on to meowcoin-cli (default: -signet)')

args = parser.parse_args()

if args.meowcoin_cli_args == []:
    args.meowcoin_cli_args = ['-signet']


def meowcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.meowcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = meowcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = meowcoin_cli(['getnewaddress', 'faucet', 'bech32'])
