...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing telestai-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='telestai-cli', help='telestai-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Telestai address to which the faucet should send')
...
parser.add_argument('telestai_cli_args', nargs='*', help='Arguments to pass on to telestai-cli (default: -signet)')

args = parser.parse_args()

if args.telestai_cli_args == []:
    args.telestai_cli_args = ['-signet']


def telestai_cli(rpc_command_and_params):
    argv = [args.cmd] + args.telestai_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = telestai_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = telestai_cli(['getnewaddress', 'faucet', 'bech32'])
