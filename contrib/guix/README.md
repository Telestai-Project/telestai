# Bootstrappable Telestai Core Builds

This directory contains the files necessary to perform bootstrappable Telestai
Core builds.
...
To build Telestai Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Telestai Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/telestai-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/telestai-core/guix.sigs).
