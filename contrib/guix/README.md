# Bootstrappable Meowcoin Core Builds

This directory contains the files necessary to perform bootstrappable Meowcoin
Core builds.
...
To build Meowcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Meowcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/meowcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/meowcoin-core/guix.sigs).
