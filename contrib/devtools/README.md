...
gen-telestai-conf.sh
===================

Generates a telestai.conf file in `share/examples/` by parsing the output from `telestaid --help`. This script is run during the
release process to include a telestai.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-telestai-conf.sh
```
